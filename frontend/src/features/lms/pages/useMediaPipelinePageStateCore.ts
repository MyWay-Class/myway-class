...
import { loadShortformExportStatus, type ShortformExportStatusSummary } from '../../../lib/api-shortforms';
...
export function useMediaPipelinePageState({ selectedCourse, highlightedLecture, sessionToken, viewerRole }: Params) {
  ...
  const [uploadResult, setUploadResult] = useState<MediaUploadResult | null>(demoMode ? buildDefaultDemoUploadResult(demoLectureDetail, demoAudioExtraction) : null);
  ...
  useEffect(() => setLectureId(defaultLectureId), [defaultLectureId]);
  useEffect(() => {
    setBannerDescription(getPublicTestPolicyText('media'));
    setBannerMeta(null);
  }, [lectureId, selectedCourse?.id]);

  const refreshMediaState = useCallback(async (targetLectureId: string, options?: { silent?: boolean }) => {
    if (demoMode) {
      ...
      setUploadResult({ ...buildDefaultDemoUploadResult(demoLectureDetail, demoAudioExtraction) });
      return;
    }
    if (!targetLectureId) {
      setPipeline(null); setExtractions([]); setTranscript(null); return;
    }
  ...
  const selectedLecture = useMemo(() => lectureOptions.find((lecture) => lecture.id === lectureId) ?? highlightedLecture ?? demoLectureDetail, [highlightedLecture, lectureId, lectureOptions]);
  const latestExtraction = useMemo(() => [...extractions].sort((left, right) => (right.updated_at ?? right.created_at).localeCompare(left.updated_at ?? left.created_at))[0] ?? null, [extractions]);
  const retrySource = useMemo(() => ({
    video_url: uploadResult?.video_url ?? latestExtraction?.source_url ?? (highlightedLecture?.id === lectureId ? highlightedLecture.video_url : undefined),
  ...
  useEffect(() => {
    if (!lectureId) return;
    const needsPolling = latestExtraction?.status === 'PROCESSING' || latestExtraction?.stt_status === 'PROCESSING' || pipeline?.audio_status === 'PROCESSING' || pipeline?.transcript_status === 'PROCESSING';
    if (!needsPolling) return;
    const timer = window.setInterval(() => { void refreshMediaState(lectureId, { silent: true }); }, 4000);
  ...
  useEffect(() => {
    if (viewerRole !== 'ADMIN' || demoMode) { setShortformExportStatus(null); return; }
    let active = true;
    loadShortformExportStatus(sessionToken).then((status) => { if (active) setShortformExportStatus(status); });
    return () => { active = false; };
  ...
  return {
    demoMode, displayCourse, lectureOptions, lectureId, setLectureId, videoFile, setVideoFile, audioUrl, setAudioUrl, busy, setBusy,
    notice, setNotice, bannerDescription, setBannerDescription, bannerMeta, setBannerMeta, pipeline, transcript, providers, processorHealth,
    uploadResult, setUploadResult, extractions, isRefreshing, speakerReview, setSpeakerReview, speakerLabel, setSpeakerLabel,
    instructorName, setInstructorName, speakerConfidence, setSpeakerConfidence, speakerNote, setSpeakerNote, shortformExportStatus, setShortformExportStatus,
    selectedLecture, latestExtraction, retrySource, refreshMediaState, requiresManualApproval, sttPolicySummary,
  };
}
