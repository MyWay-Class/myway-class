...
type LectureStudioPreviewSectionsProps = {
  draft: LectureStudioDraft;
  selectedCourse: CourseDetail | null;
  activeLecture: LectureDetail | Lecture | null;
  outlineItems: string[];
...
function formatLectureLabel(lecture: LectureDetail | Lecture | null): string {
  if (!lecture) return '강의를 선택하면 미리보기가 갱신됩니다.';
  const sessionLabel = lecture.session_number ?? lecture.order_index + 1;
  return `${lecture.week_number ?? 1}주차 · ${sessionLabel}차시`;
}

export function LectureStudioPreviewSections({ draft, selectedCourse, activeLecture, outlineItems, materialItems, statusNote }: LectureStudioPreviewSectionsProps) {
  return (
