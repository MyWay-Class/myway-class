...
- 백엔드는 Spring(`backend-spring`)을 기본 경로로 배포하고, DB는 Supabase PostgreSQL을 사용한다.
- Cloudflare Workers(`backend`)는 보조 경로로 유지한다.
...
- Spring 백엔드 DB 연결은 `MYWAY_DB_URL`, `MYWAY_DB_USERNAME`, `MYWAY_DB_PASSWORD`를 사용한다.
- Supabase pooler를 쓸 때 JDBC URL은 `jdbc:postgresql://<pooler-host>:5432/postgres?sslmode=require` 형식을 사용한다.
...
- Pages 브랜치 배포는 `VITE_API_BASE_URL`을 브랜치별로 명시해 주입해야 한다. 이 값이 없으면 `frontend/src/lib/api-base.ts`가 `.pages.dev` 호스트를 production API로 해석하므로, `dev` 브랜치 Pages는 의도한 API가 아니라 production을 바라볼 수 있다.
...
- Spring 런타임 시크릿(`MYWAY_DB_PASSWORD`, `MYWAY_AI_GEMINI_API_KEY`)은 서버/플랫폼 시크릿으로만 주입하고 코드 저장소에는 남기지 않는다.
...
6. Pages 프로젝트에 `VITE_API_BASE_URL`을 Workers API 주소로 넣고, `dev`/`main` 브랜치별 값이 서로 섞이지 않는지 확인한다.
