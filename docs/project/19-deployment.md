...
4. Pages 프로젝트에 `VITE_API_BASE_URL`을 Workers API 주소로 넣는다.
5. `npm run build:frontend`와 `npm run build:backend`를 확인한다.
6. Pages에 `frontend/dist`를 연결한다.
7. Workers에 `backend/src/index.ts`를 연결한다.
