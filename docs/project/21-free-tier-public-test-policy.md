...
## 배포 전제
- `MYWAY_GEMINI_API_KEY`, `MYWAY_MEDIA_PROCESSOR_TOKEN`, `MYWAY_MEDIA_CALLBACK_SECRET`는 vars가 아니라 secret으로 넣는다.
- D1 quota 저장소는 `DB`로 바인딩한다.
- 공개 테스트 모드는 `MYWAY_AI_PUBLIC_MODE=free_test`를 기준으로 둔다.
